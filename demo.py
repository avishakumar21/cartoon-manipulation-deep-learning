...
def process_image(img, mask, name, opt, save_to_input=True):
    ...
    #mask_raw = np.array(mask)>0
    mask = mask.resize((w_t, h_t))
    mask = np.array(mask)>0
    edge = mask[...,0]
    region = (mask[...,2]+edge)>0
    ...
    with torch.no_grad():
        generated,_ = model(
                {'image':img,'mask':region,'edge':edge},
                mode='inference')
...
def hello(name=None):
    if 'changeim' in request.form:
        ...
        W, H = image.size
        return render_template('mask.html', name=name, image_name=filename, image_width=W,
    ...
    if request.method == 'POST':
        ...
        if 'mask' in request.form:
            ...
            return render_template('mask.html', name=name, image_name=filename, #f"{args.opt[0]}_"+maskname
        ...
        image = Image.open(os.path.join(os.path.join(app.config['UPLOAD_FOLDER'], filename)))
        W, H = image.size
        return render_template('mask.html', name=name, image_name=filename, image_width=W, image_height=H,
