...
class InpaintCModel(torch.nn.Module):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        networks.modify_commandline_options(parser, is_train)
        parser.add_argument('--not_om', action='store_true',
    ...
    def forward(self, data, mode, **kwargs):
        inputs, real_image, edge, mask = self.preprocess_input(data)
    ...
    def preprocess_input(self, data):
        ...
        data['inputs'] = inputs
        return inputs, data['image'], data['edge'], mask

    def g_image_loss(self, coarse_image, fake_image, composed_image, real_image, edge):
        G_losses = {}
        if not self.opt.no_gan_loss:
            pred_fake, pred_real = self.discriminate(
                composed_image, real_image, edge)
    ...
    def compute_generator_loss(self, inputs, real_image, edge, mask):
        ...
        G_losses = self.g_image_loss(coarse_image, fake_image, composed_image, real_image, edge)


        return G_losses, coarse_image, composed_image

    def compute_discriminator_loss(self, inputs, real_image, edge, mask):
        D_losses = {}
        if not self.opt.no_gan_loss:
            with torch.no_grad():
                coarse_image, fake_image = self.generate_fake(inputs, real_image, edge, mask)
    ...
    def generate_fake(self, inputs, real_image, edge, mask):
        coarse_image, fake_image = self.netG(inputs, mask, edge)
    ...
    def discriminate(self, fake_image, real_image, edge):
        ...
        discriminator_out = self.netD(fake_and_real, edge_merge)
